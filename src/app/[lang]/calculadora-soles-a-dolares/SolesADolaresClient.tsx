...
export default function SolesADolaresClient({ initialRate }: Props) {
  ...
  const totalDolares = activeRate > 0 ? numericMonto / activeRate : 0;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-6 space-y-6">
          <div>
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-bold text-slate-400">S/</span>
              <input
                type="number"
                value={monto}
        ...
        <div className="lg:col-span-6 flex flex-col justify-between bg-gradient-to-br from-emerald-900 to-teal-950 text-white rounded-3xl p-6 sm:p-8 shadow-md">
          <div>
            <span className="text-xs font-bold uppercase tracking-widest text-emerald-200">
              Resultado en Dólares Estadounidenses
            </span>
            <div className="mt-4">
              <div className="text-sm text-emerald-200/80 font-medium">
                S/ {numericMonto.toFixed(2)} PEN son:
              </div>
              <div className="text-4xl sm:text-5xl font-black tracking-tight text-white mt-1">
                US$ {totalDolares.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                <span className="text-lg font-bold text-emerald-300 ml-2">USD</span>
              </div>
            </div>
          ...
          <div className="mt-6 text-xs text-emerald-200/80 leading-relaxed pt-4 border-t border-emerald-800/60">
            Fórmula: S/ {numericMonto.toFixed(2)} ÷ S/ {activeRate.toFixed(3)} = US$ {totalDolares.toFixed(2)} USD
          </div>
        </div>
      </div>
    </div>
